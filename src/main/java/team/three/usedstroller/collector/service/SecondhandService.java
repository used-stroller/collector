...
public class SecondhandService {
  ...
  public int collectingJunggonara() throws InterruptedException {
    ...
    for (int i = 1; i < pageTotal; i++) {
      ...
      Thread.sleep(1000);
      try {
        WebElement content = customWebDriver.findElement(
            By.xpath("//*[@id=\"__next\"]/div/main/div[1]/div[2]/ul"));
  ...
  public int collectingBunJang() throws InterruptedException {
    ...
    for (int i = 1; i < pageTotal; i++) {
      String url = "https://m.bunjang.co.kr/search/products?order=score&page=" + i
          + "&q=%EC%9C%A0%EB%AA%A8%EC%B0%A8";
      customWebDriver.openURL(url);
      Thread.sleep(1000);
  ...
  public int collectingHelloMarket() throws InterruptedException {
    int complete = 0;
  ...
  private static List<Junggo> getItemListJunggo(List<WebElement> list) {
    ...
    for (WebElement element : list) {
      ...
      Junggo junggo = Junggo.builder().title(title).link(link).price(price).imgSrc(img)
          .address(address).uploadTime(uploadTime).build();
  ...
  private static List<BunJang> getItemListBunJang(List<WebElement> list) {
    ...
    for (WebElement element : list) {
      ...
      BunJang bunJang = BunJang.builder().title(title).link(link).price(price).imgSrc(img)
          .address(address).uploadTime(uploadTime).build();
  ...
  private static List<HelloMarket> getItemListHelloMarket(List<WebElement> list) {
    ...
    for (WebElement element : list) {
      ...
      HelloMarket helloMarket = HelloMarket.builder().title(title).link(link).price(price)
          .imgSrc(img).uploadTime(uploadTime).build();
  ...
  private int getTotalPageJungGo() {
    ...
    customWebDriver.openURL(url);
    WebElement content = customWebDriver.findElement(By.xpath("//*[@id=\"__next\"]"));
    String totalQty = content.findElement(
        By.xpath("//*[@id=\"__next\"]/div/main/div[1]/div[2]/div[2]/div/div/div[1]")).getText();
  ...
  private int getTotalPageBunJang() {
    ...
    String totalQty = content.findElement(
        By.xpath("//*[@id=\"root\"]/div/div/div[4]/div/div[3]/div/div[1]/span[2]")).getText();
    String intStr = totalQty.replaceAll("[^0-9]", "");
    int totalQtyInt = Integer.parseInt(intStr);
    totalPage = totalQtyInt / qtyPerPage;
    return totalPage;
