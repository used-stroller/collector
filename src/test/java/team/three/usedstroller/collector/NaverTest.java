...
//		options.setHeadless(true);
  ...
  private List<WebElement> getProducts() {
    WebElement prodList = driver.findElement(
        By.cssSelector("#content > div.style_content__xWg5l > div.basicList_list_basis__uNBZx"));
  ...
  private boolean crawlProducts(List<WebElement> list) throws InterruptedException {
    ...
    for (WebElement el : list) {
      ...
      imgSrc = el.findElement(By.xpath(".//a[contains(@class, 'thumbnail')]/img"))
          .getAttribute("src");
      title = el.findElement(By.xpath(".//a[@title]")).getText();
      price = el.findElement(By.xpath(".//span[contains(@class, 'price')]")).getText()
          .replace("최저", "").trim();
      System.out.println(
          "link = " + link + ", imgSrc = " + imgSrc + ", title = " + title + ", price = " + price);
